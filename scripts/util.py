...
def apply_tree_bins(data, transformation_df, numeric_var):
    ...
    df_copy[numeric_var] = pd.to_numeric(df_copy[numeric_var], errors='coerce')

    # Ordena os upper bounds (já são floats)
    upper_bounds = transformation_df['Upper_Bound'].astype(float).sort_values().values

    # Aplica os bins com digitize
    df_copy[f"TFT_{numeric_var}"] = np.digitize(df_copy[numeric_var].values, upper_bounds)

    # Remove a coluna original
    df_copy.drop(columns=[numeric_var], inplace=True)
...
def logistic_regression_with_scorecard(data, target_var, features):
    """
    Ajusta uma regressão logística e gera um scorecard com os coeficientes beta, odds ratio,
    p-valores e estatísticas de Wald.
    ...
    Retorna:
        ...
        scorecard: DataFrame com Beta, Odds Ratio, P-Value e Estatística de Wald ordenados
    """
    # Garante que a variável target não está na lista de features
    ...
    summary['Odds Ratio'] = np.exp(summary['Coef.'])  # 💡 cálculo do OR

    # Monta o scorecard final
    scorecard = summary[['Coef.', 'Odds Ratio', 'P>|z|', 'Wald']]
    scorecard.columns = ['Beta Coefficient', 'Odds Ratio', 'P-Value', 'Wald Statistic']
...
# ======================================================
# 1. SEPARAÇÃO DE FEATURES NUMÉRICAS E CATEGÓRICAS
# ======================================================
...
def dataprep(X_train, X_test, y_train):
    ...
    # 2. Imputação
    Xtr_num, Xte_num, Xtr_cat, Xte_cat, imp_num, imp_cat = impute_data(
    ...
    train_final, test_final, dropped_corr_cols = drop_correlated_features(
        train_final, test_final, threshold=0.5
    )
